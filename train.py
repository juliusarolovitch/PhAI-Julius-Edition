...
def loss_function(inputs, output, target):
    mse_loss = F.mse_loss(output.squeeze(), target)
...
if __name__ == '__main__':
    datasets = []
    for i in tqdm(range(70), desc="Building training dataset..."):
        datasets.append(torch.load(os.path.join(
            'datasets', f'dataset_map_{i}.pt')))
